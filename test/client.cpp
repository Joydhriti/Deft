...
constexpr int MEASURE_SAMPLE = 1;
...
uint64_t percentile_ns_from_samples(std::vector<uint64_t> samples_[/*MAX_APP_THREAD*/][8],
                                    int num_threads, int metric, double p) {
  // compute total size and make one big buffer (so nth_element can work)
  size_t total = 0;
  for (int t = 0; t < num_threads; ++t) total += samples_[t][metric].size();
  if (total == 0) return 0;

  std::vector<uint64_t> merged;
  merged.reserve(total);
  for (int t = 0; t < num_threads; ++t) {
    merged.insert(merged.end(), samples_[t][metric].begin(), samples_[t][metric].end());
  }

  // 0 < p <= 1  (e.g., 0.99 for p99)
  size_t idx = (size_t)std::ceil(p * merged.size()) - 1;  // 0-based index
  if ((long long)idx < 0) idx = 0;
  if (idx >= merged.size()) idx = merged.size() - 1;

  std::nth_element(merged.begin(), merged.begin() + idx, merged.end());
  return merged[idx];  // nanoseconds
}

// Convenience wrappers
double percentile_us_from_samples(std::vector<uint64_t> samples_[/*MAX_APP_THREAD*/][8],
                                  int num_threads, int metric, double p) {
  return percentile_ns_from_samples(samples_, num_threads, metric, p) / 1000.0;
}
  ...
  if (id < FLAGS_num_prefill_threads) {
    ...
    for (size_t i = 0; i < prefill_keys.size(); ++i) {
      ...
      if (measure_lat) {
        auto t = timer.end();
        stat_helper.add(id, lat_op, t);
	//stat_helper.add_sample(id, lat_op, t);
      }
      // if (id == 0 && i % 100000 == 0) {
      //   printf("prefill %ld\n", i);
      //   fflush(stdout);
...
int metric = lat_op;                // your enum value
int num_threads = FLAGS_num_bench_threads;
double p90_us = percentile_us_from_samples(stat_helper.samples_, num_threads, metric, 0.90);
double p99_us = percentile_us_from_samples(stat_helper.samples_, num_threads, metric, 0.99);
double p999_us = percentile_us_from_samples(stat_helper.samples_, num_threads, metric, 0.999);
printf("p90 = %.3f us\n", p90_us);
printf("p99 = %.3f us\n", p99_us);
printf("p999 = %.3f us\n", p999_us);
#endif
